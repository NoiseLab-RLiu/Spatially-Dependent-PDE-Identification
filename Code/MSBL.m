function [X,gamma_ind,gamma_est,count,errorbar] = MSBL(Phi, Y, lambda, Learn_Lambda, varargin)
...
PRUNE_GAMMA = 1e-5;       % threshold for prunning small hyperparameters gamma_i. 1e-4
EPSILON     = 1e-8;       % threshold for stopping iteration. 
MAX_ITERS   = 500;       % maximum iterations. 2000
...
for i=1:length(gamma)
    if(mu(i)~=0)
        errorbar=errorbar+abs(Sigma_w_diag(i)/mu(i)^2);
    end
end
